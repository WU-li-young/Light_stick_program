...
#define LED_PIN     8
#define NUM_LEDS    120      // 請確認這裡是不是你的燈珠總數 (你上一版寫120, 這裡改回96預設, 請依實際修改)
#define HALF_LEDS   60      // 總數的一半
...
<head>
  ...
  <style>
    ...
    @keyframes breathe-anim { 0% { opacity: 0.4; } 50% { opacity: 1; } 100% { opacity: 0.4; } }
    ...
    .stick-part { width: 80px; height: 60px; background: #222; border: 2px solid #444; display: flex; align-items: center; justify-content: center; font-size: 14px; color: #888; font-weight: bold; transition: background 0.2s; }
    #preview-top { border-radius: 12px 12px 0 0; border-bottom: none; }
    #preview-btm { border-radius: 0 0 12px 12px; border-top: none; }

    .fx-rainbow { background: linear-gradient(270deg, #f00, #ff0, #0f0, #0ff, #00f, #f0f); background-size: 400%; animation: rainbow-anim 3s linear infinite; color: #000; }
    .fx-breathe { animation: breathe-anim 2s ease-in-out infinite; }
    .fx-chase { background: linear-gradient(to bottom, transparent 40%, #fff 50%, transparent 60%); background-size: 100% 200%; animation: chase-anim 1s linear infinite; }

    .panel { background: #1a1a1a; padding: 10px; border-radius: 15px; margin-bottom: 10px; border: 1px solid #333; }
    .mode-group { display: flex; gap: 5px; margin-bottom: 10px; }
    .mode-btn { flex: 1; padding: 10px; background: #333; border: 2px solid #444; color: #aaa; border-radius: 8px; font-weight: bold; }
    .mode-btn.active { border-color: #00e5ff; color: #000; background: #00e5ff; }

    .fx-grid { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 5px; margin-bottom: 10px; }
    .fx-btn { padding: 10px 0; background: #222; border: 1px solid #444; color: #fff; border-radius: 8px; font-size: 12px; }
    ...
    #colorWheel { width: 100%; height: 100%; cursor: crosshair; }
    ...
    .btn-off { width: 100%; padding: 12px; background: #222; border: 2px solid #ff4444; color: #ff4444; border-radius: 10px; font-size: 16px; font-weight: bold; }
  </style>
</head>
<body>
  ...
  <script>
    ...
    var canvas = document.getElementById('colorWheel'); var ctx = canvas.getContext('2d');
    ...
    function pickColor(e) {
      ...
      var pixel = ctx.getImageData(x, y, 1, 1).data; if (pixel[3] < 200) return; 
    ...
    function setTarget(t) {
      target = t; document.querySelectorAll('.mode-btn').forEach(b => b.classList.remove('active'));
    ...
    function highlightFxBtn(m) { document.querySelectorAll('.fx-btn').forEach(b => b.classList.remove('active')); document.getElementById('fx-'+m).classList.add('active'); }
  </script>
</body>
</html>
)rawliteral";
...
void drawSegment(int start, int len, int mode, CRGB staticColor, uint8_t beat, uint8_t beatSlow) {
  if (mode == 0) { fill_solid(leds + start, len, staticColor); } 
  else if (mode == 1) { fill_rainbow(leds + start, len, beat, 5); }
  else if (mode == 2) { uint8_t bri = cubicwave8(beatSlow); fill_solid(leds + start, len, staticColor); for(int i=start; i<start+len; i++) leds[i].nscale8(bri); }
  else if (mode == 3) { fill_solid(leds + start, len, CRGB::Black); int pos = beatsin16(effectSpeed, 0, len-1); leds[start + pos] = staticColor; for(int i=1; i<4; i++) { if(pos-i >= 0) leds[start + pos - i] = staticColor; leds[start + pos - i].fadeToBlackBy(i*60); } }
}
...
void setup() {
  ...
  Serial.println("");
  Serial.println("===============================");
  Serial.println("光棍已啟動 (極速版)");
  Serial.print("Wi-Fi IP 位址: http://");
  ...
  server.on("/color", []() {
    ...
    CRGB c = CRGB(r, g, b);
    if (t == 0) { colorA = c; colorB = c; } else if (t == 1) { colorA = c; } else if (t == 2) { colorB = c; }
    server.send(200, "text/plain", "OK");
  });
