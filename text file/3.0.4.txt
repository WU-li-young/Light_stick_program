...
int presetA = 6; int presetB = 7; // 預設改成 POV 特效讓你測試
...
<head>
  ...
  <style>
    ...
    .stick-part { width: 80px; height: 80px; background: #222; border: 2px solid #555; display: flex; align-items: center; justify-content: center; font-size: 16px; color: #fff; font-weight: bold; transition: background 0.1s; }
    ...
    .q-btn { 
      height: 50px; border-radius: 8px; border: 2px solid #444; cursor: pointer; 
...
<body>
  ...
  <div class="panel">
    ...
    <div id="ui-effect" class="hidden">
      ...
      <div class="row"><span>⚡ 速度</span><input type="range" min="1" max="255" value="30" oninput="setSpeed(this.value)"></div>
    </div>
  ...
  <script>
    var target = 0; 
    var state = { 
      modeA: 1, colorA: '#ff0000', presetA: 6, briA: 255,
      modeB: 1, colorB: '#ff0000', presetB: 7, briB: 255
    };
...
void runEffect(int start, int len, int presetID, CRGB baseColor) {
  ...
  switch(presetID) {
    ...
    // --- 🌀 POV 旋轉特效 (重點) ---
    case 6: // 虛線光環 (Strobe)
      // 極速開關：亮 20% 時間，暗 80% 時間
      // 轉起來會變成虛線圓圈
      if ( (millis() % (105 - effectSpeed/3)) < 15 ) {
        fill_solid(leds + start, len, CRGB::White);
      } else {
    ...
    case 7: // 紅白分段 (Segment)
      // 快速切換紅白，轉起來像救生圈
      if ( (millis() / (105 - effectSpeed/3)) % 2 == 0 ) {
        fill_solid(leds + start, len, CRGB::Red);
      } else {
        fill_solid(leds + start, len, CRGB::White);
      }
      break;

    case 8: // 漸層光帶 (Hyper Rainbow)
      // 超高速彩虹，轉起來會變成完整的彩虹圈
    ...
    case 9: // 碎形星塵 (Dot)
      ...
      fill_solid(leds + start, len, CRGB::Black);
      if(random8() < 50) {
        leds[start + random16(len)] = CHSV(random8(), 255, 255);
      }
      break;
  }
}
