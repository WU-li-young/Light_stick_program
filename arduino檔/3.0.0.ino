...
uint8_t effectSpeed = 20; // 數值越小 = 變色越慢 (注意：beat8 的邏輯是頻率)
...
<head>
  ...
  <style>
    ...
    @keyframes cycle-anim { 0% { background-color: #ff0000; } 20% { background-color: #ffff00; } 40% { background-color: #00ff00; } 60% { background-color: #00ffff; } 80% { background-color: #0000ff; } 100% { background-color: #ff0000; } }
    ...
    .fx-cycle { animation: cycle-anim 5s linear infinite; }
...
void drawSegment(int start, int len, int mode, CRGB staticColor, uint8_t beat, uint8_t beatSlow) {
  ...
  else if (mode == 4) { // 輪替 (全域變色)
    fill_solid(leds + start, len, CHSV(beat, 255, 255));
  }
}
...
void setup() {
  ...
  if(result) {
    ...
    Serial.print("熱點名稱: "); Serial.println(ap_ssid);
    Serial.print("熱點密碼: "); Serial.println(ap_password);
    Serial.print("控制網址 IP: http://"); Serial.println(WiFi.softAPIP());
    Serial.println("請用手機連線並開啟網址");
  } else {
    Serial.println("Wi-Fi 熱點建立失敗! 請檢查硬體或電源");
  }
